...
TMP_DIR="$(mktemp -d /tmp/dev-start-nextjs-prisma-ds-workflow-dev-tooling-XXXXXX)"
SOURCE_APP="$TMP_DIR/source-app"
BUILT_APP="$TMP_DIR/built-app"
DEV_LOG="$TMP_DIR/dev.log"
BUN_TMP_DIR="$ROOT_DIR/.tmp/verify-nextjs-prisma-ds-workflow-dev-tooling"
...
assert_overlay_shape() {
  ...
  echo "Validating Prisma + ds-workflow + dev-tooling output for $expected_name"
...
echo "Scaffolding Prisma + ds-workflow + dev-tooling app from source"
(
  cd "$ROOT_DIR"
  bun cli/src/index.ts -- "$SOURCE_APP" --prisma --workflow --dev-tooling -y --no-install --no-git
)

assert_overlay_shape "$SOURCE_APP" "source-app"

echo "Scaffolding Prisma + ds-workflow + dev-tooling app from built output"
(
  cd "$ROOT_DIR"
  node cli/dist/index.js "$BUILT_APP" --prisma --workflow --dev-tooling --no-install
)
...
echo "Installing generated Prisma + ds-workflow + dev-tooling app dependencies"
(
  cd "$BUILT_APP"
...
echo "Running Prisma + ds-workflow + dev-tooling setup contract"
(
  cd "$BUILT_APP"
  bun run db:generate
...
echo "Running generated Prisma + ds-workflow + dev-tooling app quality gates"
(
  cd "$BUILT_APP"
  bun run build
  bun run lint
...
echo "Checking generated Prisma + ds-workflow + dev-tooling app dev server"
(
  cd "$BUILT_APP"
  status=0
...
echo "nextjs/base + prisma + ds-workflow + dev-tooling verification passed"

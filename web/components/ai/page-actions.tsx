    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          />
          <div className="absolute right-0 z-50 mt-1 min-w-[200px] rounded-xl border bg-popover/60 p-2 text-sm text-popover-foreground shadow-lg backdrop-blur-lg">
            {items.map((item) => (
              <a
                key={item.href}
                href={item.href}
                ...
                target="_blank"
                onClick={() => setOpen(false)}
                className="inline-flex w-full items-center gap-2 rounded-lg p-2 hover:bg-accent hover:text-accent-foreground [&_svg]:size-4"
              >
                {item.icon}
                {item.title}
